...
{  
  ...
  $.ajax({
    ...
    success: function(response){  
      ...
      $('#modal-title').html("<i class='fas fa-plus'></i> | Edit Data");
    },
    error: function (xhr, ajaxOptions, thrownError) { 
      console.log(xhr.responseText);
    }
...
function kirim_ulang(tahap, no_rawat)
{  
  var data = {tahap: tahap, no_rawat: no_rawat};

  $.ajax({
    url: base_url+'link/kirim_ulang',
    type: 'POST',
    data: data,
    dataType:'json',
    success: function(response){
